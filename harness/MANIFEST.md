...
| 표준 | 현재 상태 | 필요한 하네스 |
|---|---|---|
| 앱은 서버 없이 실행 가능한 빌드 없는 정적 앱이다. | `index.html` 진입점, `assets/guide/*.png` 정적 이미지, JS/CSS 앱 로직은 아직 `index.html` 중심 | GitHub Pages 직접 배포 경로와 문서 일치 점검 |
...
| CSS/JS/vendor를 언제 분리할 것인가? | 유지보수성은 좋아지지만 파일 경로와 배포 문서가 함께 바뀐다 | 빌드 없는 정적 파일 분리만 허용하고 단계별 검토 |
